...
void PlayerDDD::DrawBullets() {

	for (int i = 0; i < m_bullets.size(); i++)
	{
		if (m_bullets[i].GetAlive())
		{
			m_bullets[i].Draw(3); // playerNo = 0
		}
	}
...
BulletBase PlayerDDD::CreateBullet() {
...
void PlayerDDD::DrawPoto() {
	...
	IDirect3DTexture9* m_pTex = GetAppInst()->GetDxTex(TEX_DDDPHTO);// テクスチャの確認
...
void PlayerDDD::DrawNation() {
	...
	IDirect3DTexture9* m_pTex = GetAppInst()->GetDxTex(TEX_DDDNATION);// テクスチャの確認
	assert(m_pTex != NULL && "ID3DTex オブジェクトが初期化されていません");
