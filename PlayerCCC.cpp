...
//CCC(攻撃)
//TankHp   3   + (0)  =4
//TankMove 10  + (2)  =10 X 1.2 = 12
//PoHP     3   + (0)  =4 
//PoMove   5   + (2)  = 5 X 1.2 = 6



//初期化
void PlayerCCC::Init(Field* f) {
	m_pfield = f;

	float fieldLeft = (WIDTH - COLS * FIELD_SIZE) / 2;
	float fieldTop = (HEIGHT - ROWS * FIELD_SIZE) / 2;

	float aaaX = fieldLeft + FIELD_SIZE / 2;
	float aaaY = fieldTop + FIELD_SIZE / 2;

	// 左上AAAの真下 = 左下
	m_tstatus.pos.x = aaaX;
	m_tstatus.pos.y = fieldTop + (ROWS - 1) * FIELD_SIZE + FIELD_SIZE / 2;
...
void PlayerCCC::Move() {
	...
	if (m_tstatus.hp > 0) {
		...
		if (pInput->IsPushBtn(JOY_CON_1, JOY_BTN_BIT_DN) && m_tstatus.pos.y <= down)
		{
			iy += 1;
		...
		if (pInput->IsPushBtn(JOY_CON_1, JOY_BTN_BIT_LT) && m_tstatus.pos.x >= left)
		{
			ix -= 1;
		...
		if (pInput->IsPushBtn(JOY_CON_1, JOY_BTN_BIT_RT) && m_tstatus.pos.x <= right)
		{
			ix += 1;
...
void PlayerCCC::DrawBullets() {

	for (int i = 0; i < m_bullets.size(); i++)
	{
		if (m_bullets[i].GetAlive())
		{
			m_bullets[i].Draw(2); // playerNo = 0
		}
	}


}
bool PlayerCCC::CheckLayOut1() {
...
BulletBase PlayerCCC::CreateBullet() {
...
Vector2 PlayerCCC::GetBulletStartPos()
{
...
void PlayerCCC::DrawPoto() {
	...
	IDirect3DTexture9* m_pTex = GetAppInst()->GetDxTex(TEX_CCCPHTO);// テクスチャの確認
...
void PlayerCCC::DrawNation() {
	...
	IDirect3DTexture9* m_pTex = GetAppInst()->GetDxTex(TEX_CCCNATION);// テクスチャの確認
	assert(m_pTex != NULL && "ID3DTex オブジェクトが初期化されていません");
	...
	const float startX = 640.0f+96.0f;
...
void PlayerCCC::DrawHpBar() {
	...
	const float startX = 736.0f + 24.0f + offsetX;
...
void PlayerCCC::DrawTankMark() {
	...
	const float startX = 736.0f + offsetX;
	const float startY = 0.0f;
